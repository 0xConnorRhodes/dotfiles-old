...
--
-- h to set start marker, k 3 times to set end and trum
-- note that the cut file is hidden. It's `.$ORIGINAL_NAME` in the same directory
-- you must rename the original trimmed file before the script will make a new one
...
local function initializeIfNeeded()
    ...
    mp.set_property("pause","no") --ensures video continues after selecting start position, which is how I prefer it
end
...
mp.set_property("pause","yes") --ensures video continues after selecting start position, which is how I prefer it
    initializeIfNeeded()
